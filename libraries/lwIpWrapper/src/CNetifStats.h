...
inline void netif_stats_sprintf(
    ...
    if(written > 0 && (size - written - 1) > 5 && stats.errors.size() > 0) {
        written = snprintf(
            buffer_iterator,
            size - (buffer - buffer_iterator),
            "err:\n"
        );
        ...
        for(const auto& [err, value]: stats.errors) {
            if((size - (buffer - buffer_iterator)) > 16) {
                written = snprintf(
                    buffer_iterator,
                    size - (buffer - buffer_iterator),
                    "\t|%6u: %5u\n",
