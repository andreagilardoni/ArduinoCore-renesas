...
#ifdef CNETIF_STATS_ENABLED
    netif_stats_sprintf(cnetif_stats_buffer, C33EthernetIface.stats, STATS_BUFFER_SIZE, (8*1e6)/(1<<20), "Mbit/s");
    // __disable_irq();
    arduino::lock();
    NETIF_STATS_RESET_AVERAGES(C33EthernetIface.stats);
    // __enable_irq();
    arduino::unlock();
...
void application() {
  ...
  case APP_STATE_IFACE_UP:
    ...
    app.tcp_client = new LWIPTCPClient;
